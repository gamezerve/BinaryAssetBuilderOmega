...
if not defined mod (
	set /P mod="Mod: "
	if not defined mod goto map_fail
) else (
	echo Mod: %mod%
)
...
set maproot=%sdk%mods\%mod%\Data\Maps\Official

if not exist "%maproot%" goto map_fail

for /D %%M in ("%maproot%\*") do (
	call :BuildMap "%%~nxM"
)
...
cd /D "%sdk%"
set mappath=%sdk%mods\%mod%\Data\Maps\Official\%map%
set outpath=%sdk%mods\%mod%\Misc\Data\Maps\Official\%map%
...
if exist "%mappath%" (
	...
	echo.
	echo Building map
	if exist "%mappath%\map.bin" del "%mappath%\map.bin" /F /Q
	if exist "%mappath%\map.imp" del "%mappath%\map.imp" /F /Q
	if exist "%mappath%\map.manifest" del "%mappath%\map.manifest" /F /Q
	if exist "%mappath%\map.relo" del "%mappath%\map.relo" /F /Q
	if exist "%mappath%\map.version" del "%mappath%\map.version" /F /Q
	if exist "%mappath%\map" rd "%mappath%\map" /S /Q
	xcopy "%mappath%\*.*" "%sdk%mods\maps\%map%" /Q /Y /I /R /S
	...
	if exist "%sdk%builtmods\mods\maps\%map%\map.manifest" (
		...
		echo .asset>%exc%
if exist "%outpath%" rd "%outpath%" /S /Q
mkdir "%outpath%"

xcopy "%sdk%builtmods\mods\maps\%map%\*.*" "%outpath%" /Q /Y /I /R /S /EXCLUDE:%exc%
if exist "%outpath%\map.bin" ren "%outpath%\map.bin" map_mod.bin
if exist "%outpath%\map.imp" ren "%outpath%\map.imp" map_mod.imp
if exist "%outpath%\map.manifest" ren "%outpath%\map.manifest" map_mod.manifest
if exist "%outpath%\map.relo" ren "%outpath%\map.relo" map_mod.relo
if exist "%outpath%\map.version" ren "%outpath%\map.version" map_mod.version

echo _mod>"%outpath%\map.version"

copy "%mappath%\*.map" "%outpath%\" /Y
copy "%mappath%\*.tga" "%outpath%\" /Y
	...
	if exist "%sdk%mods\maps\%map%" rd "%sdk%mods\maps\%map%" /S /Q
) else (
	echo.
