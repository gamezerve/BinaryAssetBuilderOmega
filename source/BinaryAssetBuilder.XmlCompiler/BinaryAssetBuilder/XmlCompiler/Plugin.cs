...
#error No game set in configuration.
    ...
    {
        if (!_extendedTypeInformations.TryGetValue(typeId, out ExtendedTypeInformation info))
        {
            if (typeId == 0x11E5CF64u)
            {
                return new ExtendedTypeInformation
                {
                    HasCustomData = false,
                    TypeId = typeId,
                    Type = typeof(StringHashTable),
                    TypeName = nameof(StringHashTable),
                    ProcessingHash = ((uint)_xmlCompilerVersion) ^ 0x2C112832u,
                    TypeHash = 0x2C112832u
                };
            }

            _tracer.TraceError("Unknown extended type id: 0x{0:X8} ({0})", typeId);
